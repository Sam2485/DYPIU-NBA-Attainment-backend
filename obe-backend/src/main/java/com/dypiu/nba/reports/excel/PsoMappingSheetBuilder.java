...
/**
 * Authoritative Excel Builder for Course Attainment Sheet #3: "PSO mapping".
 * <p>
 ...
 * (Sheet "PSO mapping", sheet3.xml).
 * <p>
 * Key Features:
 * - Uniform light-grey fill for all PSO Statement cells and all Competency cells (#F2F2F2).
 * - Exact same mapping layout as Sheet #2 (PO mapping), applied to Programme Specific Outcomes (PSOs).
 * - Dynamic PSOs (PSO1..PSO-N) and dynamic COs (CO1..CO-N).
 * - Invariant: HEADER LAST COLUMN == CONTENT LAST COLUMN (totalCols = 4 + 2 * numCO).
 */
public class PsoMappingSheetBuilder {
    ...
    // Authoritative uniform light-grey fill for all PSO cells and all Competency cells (#F2F2F2)
    public static final Color COLOR_LIGHT_GREY = new Color(242, 242, 242);

    private static final String[] DEFAULT_PSO_STATEMENTS = new String[] {
            "Professional Skills-The ability to comprehend, analyze and develop software and hardware systems and applications through research, in varying domains.",
            "Problem-Solving Skills- The ability to apply standard paradigms and strategies in software project development using open-ended programming environments to deliver a quality product.",
            "Successful Career and Entrepreneurship- adaptation of modern practical and systematic approaches in creating innovative solutions for a successful career, entrepreneurship, and a zest for higher studies."
    };
    ...
    public static Sheet build(Workbook wb, String sheetName, CourseAttainmentSnapshot snapshot, byte[] leftLogo, byte[] rightLogo) {
        Sheet sheet = wb.createSheet(sheetName != null ? sheetName : "PSO mapping");
        ...
        sheet.setColumnWidth(0, (int) (38.5 * 256));   // Col A: Programme Specific Outcomes
        sheet.setColumnWidth(1, (int) (51.7 * 256));   // Col B: Competency
        sheet.setColumnWidth(2, (int) (1.738 * 256));  // Col C: Separator
        for (int i = 0; i < numCO; i++) {
            sheet.setColumnWidth(3 + i, (int) (9.438 * 256)); // Keywords CO columns
        }
        int sepCol2 = 3 + numCO;
        sheet.setColumnWidth(sepCol2, (int) (1.65 * 256)); // Separator
        for (int i = 0; i < numCO; i++) {
        ...
        int rowIdx = CourseExcelHeaderRenderer.renderHeader(
                ...
                "Course Outcome - Programme Specific Outcome (CO - PSO) Mapping",
                totalCols,
                leftLogo,
                rightLogo,
                true);
        ...
        cProg.setCellValue("Programme Specific Outcomes");
        ...
        // =========================================================================
        // PSO BLOCKS (Competency rows followed by 3 Summary rows per PSO)
        // =========================================================================
        List<PsoModel> psoModels = resolvePsoModels(snapshot);

        for (int psoIdx = 0; psoIdx < psoModels.size(); psoIdx++) {
            PsoModel pso = psoModels.get(psoIdx);
            CellStyle psoStatementStyle = s.psoStatement;
            CellStyle compStatementStyle = s.compStatement;

            List<CompModel> comps = pso.competencies;
            int numComps = Math.max(1, comps.size());
            int startPsoRow = rowIdx;
            ...
            for (int k = 0; k < numComps; k++) {
                ...
                // Col 0: PSO Statement (set text on first row, merged later)
                Cell c0 = getOrCreateCell(r, 0);
                c0.setCellStyle(psoStatementStyle);
                if (k == 0) {
                    c0.setCellValue(formatPsoStatement(psoIdx + 1, pso.code, pso.statement));
                }
            ...
            // Vertical merge of Col A across competency rows of this PSO
            if (numComps > 1) {
                CellRangeAddress psoMerge = new CellRangeAddress(startPsoRow, startPsoRow + numComps - 1, 0, 0);
                sheet.addMergedRegion(psoMerge);
                applyMergedBorders(sheet, psoMerge);
            }

            // ---------------------------------------------------------------------
            // SUMMARY ROW 1: No of competencies from given PSO mapped by COs (ht = 30 pt)
            // ---------------------------------------------------------------------
            ...
            getOrCreateCell(rSum1, kwStartCol).setCellValue("No of competencies from given " + pso.code + " mapped by COs");
            if (kwEndCol > kwStartCol) {
            ...
            for (int j = 0; j < numCO; j++) {
                ...
                if (count == 0 && comps.isEmpty()) {
                    Integer strength = findStrengthInTable1(snapshot, coItem, pso.code);
            ...
            // ---------------------------------------------------------------------
            // SUMMARY ROW 2: % of competencies from given PSO mapped by COs (ht = 30 pt)
            // ---------------------------------------------------------------------
            ...
            getOrCreateCell(rSum2, kwStartCol).setCellValue("% of competencies from given " + pso.code + " mapped by COs");
            if (kwEndCol > kwStartCol) {
            ...
            // ---------------------------------------------------------------------
            // SUMMARY ROW 3: Mapping strength of PSO of CO (ht = 30 pt)
            // ---------------------------------------------------------------------
            ...
            getOrCreateCell(rSum3, kwStartCol).setCellValue("Mapping strength of " + pso.code + " of CO");
            if (kwEndCol > kwStartCol) {
    ...
    private static Integer findStrengthInTable1(CourseAttainmentSnapshot snapshot, CourseOutcomeOrderHelper.CourseOutcomeItem coItem, String psoCode) {
        ...
        for (CourseAttainmentSnapshot.CoMappingRow r : snapshot.getTable1Mapping()) {
            if (r.getCoCode() != null && (r.getCoCode().equalsIgnoreCase(coItem.getActualCode()) || r.getCoCode().equalsIgnoreCase(coItem.getAcronym()))) {
                if (r.getPsoMappings() != null && r.getPsoMappings().containsKey(psoCode)) {
                    return r.getPsoMappings().get(psoCode);
                }
            }
        }
        return null;
    ...
    private static Integer findStrengthInTable1(CourseAttainmentSnapshot snapshot, String coCode, String psoCode) {
    ...
    private static String formatPsoStatement(int index, String psoCode, String statement) {
        if (statement == null || statement.isBlank()) {
            int defaultIdx = index - 1;
            if (defaultIdx >= 0 && defaultIdx < DEFAULT_PSO_STATEMENTS.length) {
                return DEFAULT_PSO_STATEMENTS[defaultIdx];
            }
            return psoCode;
        }
    ...
    private static List<PsoModel> resolvePsoModels(CourseAttainmentSnapshot snapshot) {
        List<PsoModel> list = new ArrayList<>();
        Map<String, CourseAttainmentSnapshot.PsoDetailRow> detailMap = new LinkedHashMap<>();
        if (snapshot.getPsoDetails() != null) {
            for (CourseAttainmentSnapshot.PsoDetailRow d : snapshot.getPsoDetails()) {
                if (d.getPsoCode() != null) {
                    detailMap.put(d.getPsoCode().toUpperCase().trim(), d);
                }
            }
        }

        List<String> psoCodes = snapshot.getPsoCodes();
        if (psoCodes == null || psoCodes.isEmpty()) {
            if (!detailMap.isEmpty()) {
                psoCodes = new ArrayList<>(detailMap.keySet());
            } else {
                psoCodes = List.of("PSO1", "PSO2", "PSO3");
            }
        }

        for (int i = 0; i < psoCodes.size(); i++) {
            String code = psoCodes.get(i).trim();
            CourseAttainmentSnapshot.PsoDetailRow detail = detailMap.get(code.toUpperCase());

            String stmt = (detail != null && detail.getStatement() != null && !detail.getStatement().isBlank())
                    ? detail.getStatement()
                    : (i < DEFAULT_PSO_STATEMENTS.length ? DEFAULT_PSO_STATEMENTS[i] : code);
            ...
            list.add(new PsoModel(code, stmt, comps));
        }

        return list;
    }

    private static class PsoModel {
        final String code;
        final String statement;
        final List<CompModel> competencies;

        PsoModel(String code, String statement, List<CompModel> competencies) {
            this.code = code;
    ...
    private static StyleBundle createStyles(Workbook wb) {
        ...
        // Uniform Light-Grey styles for PSO statement and Competency statement
        b.psoStatement = createStyle(wb, "Verdana", (short) 10, false, HorizontalAlignment.LEFT, VerticalAlignment.CENTER, true, COLOR_LIGHT_GREY, true);
