...
public class PsoMappingSheetBuilder {
    ...
    public static Sheet build(Workbook wb, String sheetName, CourseAttainmentSnapshot snapshot, byte[] leftLogo, byte[] rightLogo) {
        ...
        sheet.setColumnWidth(0, (int) (35.0 * 256)); // Col A: Programme Specific Outcomes
        sheet.setColumnWidth(1, (int) (47.0 * 256)); // Col B: Competency
        sheet.setColumnWidth(2, (int) (1.58 * 256)); // Col C: Separator
        for (int i = 0; i < numCO; i++) {
            sheet.setColumnWidth(3 + i, (int) (8.58 * 256)); // Keywords CO columns
        }
        int sepCol2 = 3 + numCO;
        sheet.setColumnWidth(sepCol2, (int) (1.5 * 256)); // Separator
        for (int i = 0; i < numCO; i++) {
        ...
        for (int psoIdx = 0; psoIdx < psoModels.size(); psoIdx++) {
            ...
            for (int j = 0; j < numCO; j++) {
                ...
                if (count == 0 && comps.isEmpty()) {
                    Integer strength = findStrengthInTable1(snapshot, coCode, pso.code);
