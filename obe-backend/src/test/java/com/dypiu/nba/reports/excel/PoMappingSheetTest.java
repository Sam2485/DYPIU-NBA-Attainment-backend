...
class PoMappingSheetTest {

    private final ExcelReportRenderer renderer = new ExcelReportRenderer();

    @Test
    @DisplayName("Case A: Standard 6 COs + 12 POs -> 16 columns (A to P), exact sheet structure, colors & widths")
    void testCaseA_Standard6Cos12Pos() throws Exception {
        ...
        try (Workbook wb = new XSSFWorkbook(new ByteArrayInputStream(bytes))) {
            ...
            assertWidthClose(32.175, sheet.getColumnWidth(0) / 256.0);
            ...
            assertTrue(matchesColor(r5.getCell(0).getCellStyle(), PoMappingSheetBuilder.COLOR_BANNER_CYAN));
            ...
            assertTrue(matchesColor(r5.getCell(3).getCellStyle(), PoMappingSheetBuilder.COLOR_BRIGHT_BLUE));
            ...
            assertTrue(matchesColor(r5.getCell(10).getCellStyle(), PoMappingSheetBuilder.COLOR_ACCENT_CYAN));

            // 4. Column Headers Row (Row 6)
            Row r6 = sheet.getRow(6);
            assertNotNull(r6);
            assertEquals(15.0f, r6.getHeightInPoints(), 0.1f);
            assertEquals("Programme Outcomes", getCellString(sheet, 6, 0));
            assertEquals("Competency", getCellString(sheet, 6, 1));
            assertTrue(matchesColor(r6.getCell(0).getCellStyle(), PoMappingSheetBuilder.COLOR_HEADER_BG));
            assertTrue(matchesColor(r6.getCell(1).getCellStyle(), PoMappingSheetBuilder.COLOR_HEADER_BG));

            for (int i = 0; i < 6; i++) {
                assertEquals("CO" + (i + 1), getCellString(sheet, 6, 3 + i));
                assertEquals("CO" + (i + 1), getCellString(sheet, 6, 10 + i));
                assertTrue(matchesColor(r6.getCell(3 + i).getCellStyle(), PoMappingSheetBuilder.COLOR_HEADER_BG));
                assertTrue(matchesColor(r6.getCell(10 + i).getCellStyle(), PoMappingSheetBuilder.COLOR_HEADER_BG));
            }

            // 5. PO1 Block (Rows 7 to 10 for competencies, Rows 11 to 13 for summary)
            // Competency rows and PO statement cells must have UNIFORM light-grey fill (#F2F2F2)
            Row r7 = sheet.getRow(7);
            assertNotNull(r7);
            assertTrue(getCellString(sheet, 7, 0).contains("PO1"));
            assertTrue(matchesColor(r7.getCell(0).getCellStyle(), PoMappingSheetBuilder.COLOR_LIGHT_GREY),
                    "PO Statement cell must have uniform light-grey fill");
            assertTrue(matchesColor(r7.getCell(1).getCellStyle(), PoMappingSheetBuilder.COLOR_LIGHT_GREY),
                    "Competency cell must have uniform light-grey fill");

            // Summary Rows for PO1 (Rows 11, 12, 13)
            Row rSum1 = sheet.getRow(11);
            assertNotNull(rSum1);
            assertEquals("No of competencies from given PO1 mapped by COs", getCellString(sheet, 11, 3));
            assertTrue(matchesColor(rSum1.getCell(3).getCellStyle(), PoMappingSheetBuilder.COLOR_SUM_ROW1));
            // CO1 count should be 4
            assertEquals("4", getCellString(sheet, 11, 10));

            Row rSum2 = sheet.getRow(12);
            assertNotNull(rSum2);
            assertEquals("% of competencies from given PO1 mapped by COs", getCellString(sheet, 12, 3));
            assertTrue(matchesColor(rSum2.getCell(3).getCellStyle(), PoMappingSheetBuilder.COLOR_SUM_ROW2));
            // CO1 pct should be 100
            assertEquals("100", getCellString(sheet, 12, 10));

            Row rSum3 = sheet.getRow(13);
            assertNotNull(rSum3);
            assertEquals("Mapping strength of PO1 of CO", getCellString(sheet, 13, 3));
            assertTrue(matchesColor(rSum3.getCell(3).getCellStyle(), PoMappingSheetBuilder.COLOR_SUM_ROW3));
            // CO1 strength should be 2 (from table1Mapping for i=1)
            assertEquals("2", getCellString(sheet, 13, 10));
        }
    }

    @Test
    @DisplayName("Case B: Dynamic 5 COs + 8 POs -> 14 columns (A to N), exact header parity")
    void testCaseB_Dynamic5Cos8Pos() throws Exception {
        ...
        try (Workbook wb = new XSSFWorkbook(new ByteArrayInputStream(bytes))) {
            Sheet sheet = wb.getSheetAt(1);
            assertEquals("PO mapping", sheet.getSheetName());
    ...
    @Test
    @DisplayName("Case C: Dynamic 8 COs + 10 POs -> 20 columns (A to T), exact header parity")
    void testCaseC_Dynamic8Cos10Pos() throws Exception {
    ...
    private CourseAttainmentSnapshot createSnapshot(int coCount, int poCount, int psoCount) {
        ...
        // Create PO details with competencies
